...
function onTextareaInput(id) {
  ...
  if (!v) { hideFeedback(id); return; }
  if (v.length < 10) {
    showFeedback(id, '💡 もう少し具体的に書いていただけると、より伝わりやすくなります。', 'tip');
  } else {
    showFeedback(id, '✏️ ありがとうございます。とても参考になります！', 'good');
  }
}
function onIdeaInput() {
  const v = getVal('q10');
  if (!v) { hideFeedback('q10'); return; }
  if (v.length < 20) {
    showFeedback('q10', '💡 思いつく範囲で、もう少し詳しく書いてみてもOKです。', 'tip');
  } else {
    showFeedback('q10', '✨ とても具体的なアイデアです！ありがとうございます。', 'good');
  }
}
